#!/usr/bin/env bash
# RTX PRO 6000 paid-run gate. Run on the Pod after prepare_env finishes.
set -euo pipefail
...
export INSTALL_BLACKWELL_SHIM="${INSTALL_BLACKWELL_SHIM:-1}"
...
log() { printf '\033[1;36m[pro6000-preflight]\033[0m %s\n' "$*"; }
err() { printf '\033[1;31m[pro6000-preflight]\033[0m %s\n' "$*" >&2; }

log "profile=${PROFILE}"
log "service=${M2SVID_SERVICE_ROOT}"
log "output=${M2SVID_OUTPUT_ROOT}"

log "1. nvidia-smi + Blackwell capability"
...
log "3. torch/cuda Pro6000 smoke"
"${APP_PY}" - <<'PY'
import sys
import torch
...
if cap[0] != 12:
    print(f"expected Blackwell/Pro6000 cap major 12, got {cap}", file=sys.stderr)
    sys.exit(12)
if mem_gb < 80:
    print(f"expected about 96GB VRAM, got {mem_gb:.1f}GB", file=sys.stderr)
    sys.exit(14)
...
log "5. Blackwell xformers shim attention smoke"
"${APP_PY}" - <<'PY'
import sys
import torch
